...
import { useCapitalsQuiz, QUIZ_CATEGORIES, getCategoryCount, type QuizCategory } from "@/application/useCapitalsQuiz";
...
function StartScreen() {
  const startGame = useCapitalsQuiz((s) => s.startGame);
  ...
  return (
    ...
    >
      ...
      <p className="mb-8 text-center text-slate-500">
        Can you name the capital of each country?
      </p>
...
function GameScreen() {
  ...
  // Focus input when a new round starts
  useEffect(() => {
  ...
  const handleSubmit = useCallback(
    ...
    [inputValue, submitGuess, currentCountry, flyToCountry, highlightCountry, triggerFeedback]
  );
    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div
            className={cn(
              "w-full rounded-2xl border bg-navy px-6 py-8 text-center transition-all duration-300",
              revealed
                ? lastGuess?.isCorrect
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">{currentCountry.continent}</p>
          </div>
        </motion.div>
      </AnimatePresence>
      ...
      <AnimatePresence>
        {revealed && (
          ...
          >
            ...
            {!lastGuess?.isCorrect && lastGuess?.guess !== "(skipped)" && (
              <p className="mt-1 text-sm text-slate-500">
                Your answer: <span className="text-wrong">{lastGuess?.guess}</span>
              </p>
            )}
          </motion.div>
        )}
        ...
        <div className="space-y-3">
          <form onSubmit={handleSubmit}>
            <input
              ...
              autoComplete="off"
              className="w-full rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-white placeholder-slate-600 outline-none transition-colors focus:border-green/40 focus:bg-white/[0.05]"
            />
          </form>
          <button
            onClick={handleSkip}
...
function ResultsScreen() {
  const { score, totalRounds, guesses, resetGame, startGame, category } = useCapitalsQuiz();
        ...
        {guesses.map((g, i) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className={cn(
                "text-xs",
                g.isCorrect ? "text-green/70" : "text-wrong/70"
              )}>
                {g.country.capital}
              </div>
              {!g.isCorrect && g.guess !== "(skipped)" && (
                <div className="text-xs text-slate-600">Your answer: {g.guess}</div>
              )}
            </div>
...
export default function CapitalsPage() {
  const { isPlaying, isFinished } = useCapitalsQuiz();
