...
import { useFlagQuiz, QUIZ_CATEGORIES, getCategoryCount, type QuizCategory, type FlagInputMode } from "@/application/useFlagQuiz";
  ...
  return (
    ...
    >
      ...
      <p className="mb-8 text-center text-slate-500">
        Can you identify the country from its flag?
      </p>
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-2">
          {([
            { id: "choice" as const, label: "Multiple Choice" },
            { id: "type" as const, label: "Type Answer" },
          ]).map((mode) => (
            <button
              key={mode.id}
      ...
      <Button
        onClick={() => {
          resetGlobe();
          startGame(rounds, selectedCategory, inputMode);
        }}
        size="lg"
        className="w-full"
      >
...
function GameScreen() {
  ...
  const handleGuess = useCallback(
    (guess: string) => {
      if (inputMode === "choice") setSelectedCode(guess);
    ...
    [submitGuess, currentCountry, flyToCountry, highlightCountry, triggerFeedback, inputMode]
  );
  ...
  }, [revealAnswer, currentCountry, flyToCountry, highlightCountry, triggerFeedback]);
    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div
            className={cn(
              "rounded-2xl border bg-navy px-10 py-6 transition-all duration-300",
              revealed
                ? lastGuess?.isCorrect
          ...
          >
            <span className="block text-center text-[120px] leading-none">
              {currentCountry.flag}
            </span>
          </div>
        </motion.div>
      ...
      {!revealed ? (
          ...
          {inputMode === "choice" ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {choices.map((choice) => (
                <button
                  key={choice.code}
                  onClick={() => handleGuess(choice.code)}
                  disabled={revealed}
                  className={cn(
                    "rounded-xl border px-4 py-3 text-sm font-semibold transition-all",
                    "border-white/10 bg-white/[0.03] text-slate-200",
                    "hover:border-green/30 hover:bg-green/5 hover:text-white",
                    "active:scale-[0.97]"
                  )}
                >
                  {choice.name}
            ...
            <CountryInput
              ...
              placeholder="Which country's flag is this?"
            />
          )}
          <button
            onClick={handleSkip}
        ...
        <div className="space-y-3">
          {/* Show MC buttons in revealed state with correct/wrong highlights */}
          {inputMode === "choice" && (
            <div className="grid grid-cols-2 gap-3">
              {choices.map((choice) => {
                const isCorrectChoice = choice.code === currentCountry.code;
                const isWrongGuess = selectedCode === choice.code && !isCorrectChoice;
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "rounded-xl border px-4 py-3 text-sm font-semibold transition-all",
                      isCorrectChoice
                        ? "animate-pulse border-green/60 bg-green/15 text-green"
                        : isWrongGuess
...
function ResultsScreen() {
  const { score, totalRounds, guesses, resetGame, startGame, category, inputMode } = useFlagQuiz();
    ...
    saveGame({
      ...
      metadata: {
        ...
        guesses: guesses.map((g) => ({
          guess: g.guess,
          answer: g.country.code,
          correct: g.isCorrect,
        })),
      },
    });
    for (const g of guesses) {
      highlightCountry(g.country.code, g.isCorrect ? "#00e676" : "#ff5252");
    }
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const handlePlayAgain = () => {
    resetGlobe();
    startGame(totalRounds, category, inputMode);
  };
      ...
      <div className="mb-6 max-h-[300px] space-y-2 overflow-y-auto pr-2">
        {guesses.map((g, i) => (
          <motion.button
            ...
            onClick={() => flyToCountry(g.country.lat, g.country.lng)}
            className={cn(
          ...
          >
            <span className="text-lg">{g.country.flag}</span>
            <div className="flex-1">
              <div className="text-sm font-medium text-white">{g.country.name}</div>
...
export default function FlagQuizPage() {
  const { isPlaying, isFinished } = useFlagQuiz();
